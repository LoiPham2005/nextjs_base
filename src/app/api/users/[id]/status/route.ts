...
export async function PATCH(request: Request, { params }: RouteContext) {
  try {
    ...
    const body = await parseJsonBody(request, updateUserStatusSchema);

    const user = await userService.setStatus(id, body.status, session.sub);

    return apiOk({ user });
  } catch (error) {
    return handleApiError(error, { route: "PATCH /api/users/[id]/status" });
  }
}
