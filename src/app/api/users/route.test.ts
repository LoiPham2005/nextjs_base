...
type ListBody = {
  data: { users: unknown[]; pagination: { page: number; perPage: number; totalPages: number } };
};
...
describe("GET /api/users", () => {
  ...
  it("200 kèm phân trang khi là ADMIN", async () => {
    vi.mocked(userService.count).mockResolvedValue(45);

    const response = await get(await signSession(admin), "?page=2&perPage=20");
    const body = (await response.json()) as ListBody;

    expect(response.status).toBe(200);
    expect(body.data.pagination).toMatchObject({ page: 2, perPage: 20, totalPages: 3 });
    expect(vi.mocked(userService.list).mock.calls[0]?.[0]).toEqual({ skip: 20, take: 20 });
  });
